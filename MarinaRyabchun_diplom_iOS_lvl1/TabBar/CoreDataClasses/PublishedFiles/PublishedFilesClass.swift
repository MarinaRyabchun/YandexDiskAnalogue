...
class PublishedFileClass {
        ...
        do {
            ...
            if arrayFetched.isEmpty && !array.isEmpty {
                array.forEach { file in
                  let newFile = PublishedFiles.init(entity: NSEntityDescription.entity(forEntityName: "PublishedFiles", in: CoreDataManager.shared.context)!, insertInto: CoreDataManager.shared.context)
                ...
                let fetchRequestDelete = PublishedFiles.fetchRequest()
    ...
    func connectContext() -> [Items]{
        ...
        let fetch = PublishedFiles.fetchRequest()
